...
## Other Helpful Information
 ...
 - The launch process creates 4 AWS resources that aren't deleted when the stack is deleted: an EIP used by the Master and ECL Watch, a ssh keypair, an s3 bucket, and volumes that are attached to each instance of the cluster. These resources must be deleted manually AFTER the stack has been deleted (if you want them deleted). And, by the way, these resources can be found using the stack name because the stack name is the resource's Name.
